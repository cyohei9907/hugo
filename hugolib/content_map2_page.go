...
func (m *pageMap) createMissingTaxonomyNodes() error {
	...
	m.taxonomyEntries.Walk(func(s string, v interface{}) bool {
		n := v.(*contentNode)
		vi := n.viewInfo
		k := m.cleanKey(vi.name.plural + "/" + vi.termKey)
...
func (m *pageMap) createSiteTaxonomies() error {
	...
	m.taxonomies.Walk(func(s string, v interface{}) bool {
		...
		if t.termKey == "" {
			...
			m.taxonomyEntries.WalkPrefix(s+"/", func(s string, v interface{}) bool {
				b2 := v.(*contentNode)
				info := b2.viewInfo
...
func (m *pageMap) assemblePages() error {
	...
	m.pages.Walk(func(s string, v interface{}) bool {
		...
		if (n.p.Kind() == page.KindPage && m.cfg.pageDisabled) || !m.s.shouldBuild(n.p) {
			m.deletePage(s)
			return false
		}
...
func (m *pageMap) assembleTaxonomies() error {
	...
	m.taxonomies.Walk(func(s string, v interface{}) bool {
		...
		kind := b.viewInfo.kind()
		sections := b.viewInfo.sections()
		...
		if b.fi != nil {
			var err error
			b.p, err = m.newPageFromContentNode(b, parent.p.bucket, nil)
			if err != nil {
				return true
			}
		} else {
			title := ""
			if kind == page.KindTaxonomy {
				title = b.viewInfo.term()
			}
			b.p = m.s.newPage(b, parent.p.bucket, kind, title, sections...)
		}

		if !m.s.shouldBuild(b.p) {
			taxonomiesToDelete = append(taxonomiesToDelete, s)
			return false
		}

		b.p.treeRef = &contentTreeRef{
			m:   m,
			t:   m.taxonomies,
			n:   b,
			key: s,
...
func (m *pageMap) attachPageToViews(s string, b *contentNode) {
	...
	// TODO(bep)
	// Index cache:
	// Main: /categories/funny/_p1
	// Reverse /_p1/categories/funny
	...
	for _, viewName := range m.cfg.taxonomyConfig {
		...
		for _, v := range vals {
			...
			m.taxonomyEntries.Insert(m.cleanKey(path.Join(viewName.plural, termKey, s)), bv)
		}
	}
}
...
func (m *pageMaps) withEveryBundle(fn func(n *contentNode) bool) {
...
type pagesMapBucket struct {
	...
	view bool // TODO1 bep chech usage for this and the rest below.
...
func (w *sectionWalker) walkLevel(prefix string, createVisitor func() sectionWalkHandler) sectionWalkHandler {
	...
	w.m.taxonomies.WalkPrefix(prefix, func(s string, v interface{}) bool {
		...
		if currentLevel == 1 {
			...
			w.m.taxonomyEntries.WalkPrefix(s, func(s string, v interface{}) bool {
				n := v.(*contentNode)
				w.err = visitor.handlePage(s, n)
				return w.err != nil
			})
		}

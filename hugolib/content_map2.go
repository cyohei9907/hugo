...
//
// Pages that belongs to a sections are stored in the `pages` tree below
...
func newContentMap(cfg contentMapConfig) *contentMap {
	...
	m.pagesByName = &contentTreeReverseIndex{
		t: m.pages,
		initFn: func(t *contentTree, m map[interface{}]string) {
			t.Walk(func(s string, v interface{}) bool {
				k := strings.TrimSuffix(path.Base(s), cmLeafSeparator)
				existing, found := m[k]
				if found && existing != ambiguityKey {
					m[k] = ambiguityKey
				} else if !found {
					m[k] = s
				}
...
func (b *cmInsertKeyBuilder) Insert(n *contentNode) *cmInsertKeyBuilder {
	if b.err == nil {
		b.tree.Insert(b.m.cleanKey(b.key), n)
	}
	return b
}

func (b *cmInsertKeyBuilder) WithFile(fi hugofs.FileMetaInfo) *cmInsertKeyBuilder {
	// TODO2 fmt.Println("WithFile:", fi.Meta().Path())
	b.newTopLevel()
	m := b.m
	meta := fi.Meta()
	p := m.cleanKey(meta.Path())
...
type contentMap struct {
	...
	// The trees where all the top level pages are stored.
	pageTrees contentTrees

	// Is the trees in pageTrees, but also includes resources.
	bundleTrees contentTrees

	// Bundles keyed by its dir or base filename,
	// e.g. "blog/post.md" => "/blog/post", "blog/post/index.md" => "/blog/post"
	pages *contentTree

	// Pages by their base filename.
	pagesByName *contentTreeReverseIndex
...
func (m *contentMap) AddFilesBundle(header hugofs.FileMetaInfo, resources ...hugofs.FileMetaInfo) error {
	...
	if isBranch {
		...
		if tc := m.cfg.getTaxonomyConfig(section); !tc.IsZero() {
			var termKey string
			termOrig := strings.TrimPrefix(strings.TrimPrefix(section, "/"+tc.plural), "/")
			if termOrig != "" {
				// TODO1 key?
				termKey = termOrig
			}
		...
		b = b.WithSection(section).ForPage(bundlePath)
		b.Insert(n)
	}

	for _, r := range resources {
		rb := b.ForResource(r.Meta().Path())
...
func (m *contentMap) DeletePrefix(prefix string) error {
	m.pages.DeletePrefix(prefix)
	m.sections.DeletePrefix(prefix)
	return nil
}
...
func (m *contentMap) getBundleDir(meta hugofs.FileMeta) string {
	dir := m.cleanKey(filepath.Dir(meta.Path()))
...
func (m *contentMap) newContentNodeFromFi(fi hugofs.FileMetaInfo) *contentNode {
	return &contentNode{
		...
		path: strings.TrimPrefix(filepath.FromSlash(fi.Meta().Path()), "/"),
	}
}
...
func (m *contentMap) cleanKey(k string) string {
...
func (m *contentMap) deleteSection(s string) {
	m.sections.Delete(s)
...
type contentTreeReverseIndex struct {
	...
	init   sync.Once
	initFn func(*contentTree, map[interface{}]string)
}

func (c *contentTreeReverseIndex) Get(key interface{}) string {
	c.init.Do(func() {
		c.m = make(map[interface{}]string)
		c.initFn(c.t, c.m)
	})
	return c.m[key]
}
